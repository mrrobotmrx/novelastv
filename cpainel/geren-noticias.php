...
if(isset($_GET["id"])){
    ...
    echo "<div style='margin-top:10px;' class='certo'>Notícia Excluido com Sucesso</div>"; 
}

        $query = $con->prepare('SELECT * FROM noticias ORDER BY id DESC LIMIT 100');
        $query->execute();
    ...
    <div class="aewf4">
        <div class="fotoresnot">
            <img src="<?php echo $result["foto"]; ?>" class="srgrgerg44">
        </div>
        <div class="tituloresu">
        <p><?php echo $result["titulo"]; ?></p>
        <span><?php echo substr($result["conteudo"], 0, 150); ?></span>
        </div>
